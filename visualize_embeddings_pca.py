...
"""
PCA visualization of classifier weights and bias (keyword) embeddings.

Projects the ERM classifier weight vectors and the top-ranked keyword
embeddings into 2D via PCA, coloring each keyword by its most-aligned class.

Usage:
    python visualize_embeddings_pca.py --dataset Waterbirds --only_spurious
    python visualize_embeddings_pca.py --dataset CelebA --top_n_biases 30
"""
...
def visualize_weights_and_biases(
    ...
    # Combine weights and bias embeddings for PCA
    ...
    ax.set_title(f"{dataset}: Classifier Weights and Top {top_n_biases} Bias Embeddings\n"
    ...
    output_path = os.path.join(output_dir, f"weights_and_biases_pca{ending}.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PCA visualization of weights and bias embeddings")
    ...
    visualize_weights_and_biases(args.dataset, args.only_spurious, args.top_n_biases, args.label_top_n)
